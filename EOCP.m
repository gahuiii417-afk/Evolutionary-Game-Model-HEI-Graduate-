...
num_cases = length(E2_diff_values);
...
for i = 1:num_cases
    ax = nexttile;
    hold(ax, 'on'); 
    current_E2_diff = E2_diff_values(i);
    [x, y] = meshgrid(0:0.02:1, 0:0.02:1);
    dx = x .* (1 - x) .* (y * dE1 - C1);
    dy = y .* (1 - y) .* (x .* (dC2 * beta - dC2 + dC2_prime) + current_E2_diff + (C2_prime - C2 - beta * dC2));
    ...
    x_star = (-current_E2_diff - (C2_prime - C2 - beta * dC2)) / (dC2 * beta - dC2 + dC2_prime);
    if x_star > 0 && x_star < 1 && y_star > 0 && y_star < 1
        plot(ax, x_star, y_star, 'kd', 'MarkerFaceColor', 'w', 'MarkerSize', 10);
    end
    axis(ax, [0 1 0 1]);
    box(ax, 'on');
    grid(ax, 'on');
    ...
    title(ax, sprintf('E_{2} - E''_{2} = %d', current_E2_diff), 'FontSize', 14);
end


title(tlo, 'System Evolution under Different E_{2} - E''_{2} Scenarios', 'FontSize', 20, 'FontWeight', 'bold');
